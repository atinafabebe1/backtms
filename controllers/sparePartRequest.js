...
//@desc      to get single spare part requests
//@route     GET/http://localhost:3500/SparePart/:ID
//@access    MECHANIC/GARAGEDIRECTOR
const getSparePart = asyncHandler(async (req, res) => {
  const { id } = req.params;

  const sparePartRequest = await SparePart.findById(id).populate({
    path: "user",
  ...
  if (!sparePartRequest) {
    return next(
      new ErrorResponse(`Spare part request not found with id of ${id}`, 404)
    );
  }
...
const statusSparePart = asyncHandler(async (req, res, next) => {
  let sparePartRequest = await SparePart.findById(req.params.id);
